...
private[yarn] class YarnAllocator(
  ...
  private val errorContainerIdToHost = new HashMap[ContainerId,Option[String]]
  private val containerIdToHost = new HashMap[ContainerId, String]
  val containerIdToRack = new mutable.HashMap[ContainerId,List[String]]()
  ...
  //for yarn-client
  def yarnClientStart = try{
    yarnClient.init(yarnConf)
    yarnClient.start()
  }catch {
    case ex => logInfo("hgs--start yarnclient error."+ex)
  }
  ...
  def allocateResources(): Unit = synchronized {
    logInfo("hgs---------------inside method allocate Resources.")
  ...
  def newContainerRequestWithPriority( resource: Resource,
                                       nodes: Array[String],
                                       racks: Array[String],
                                       relaxLocality:Boolean=true) ={
    new ContainerRequest(resource, nodes, racks, RM_REQUEST_PRIORITY_RESTART, relaxLocality, labelExpression.orNull)
  }
  ...
  def updateResourceRequests(): Unit = {
    ...
    if(hasFinishOnce&&errorContainerIdToHost.size>0){
      logInfo("hgs--missing-size:"+missing +"--error container size :"+errorContainerIdToHost.size)
    }
    ...
    logInfo("hgs-----now is in updateResourceRequests method.hasFinishOnce="+hasFinishOnce)
    if (missing > 0) {
      ...
      containerLocalityPreferences.foreach {
        case ContainerLocalityPreferences(nodes, racks) if nodes != null =>
          if(hasFinishOnce&&errorContainerIdToHost.iterator.hasNext){
            val relaxLocality = false;
            logInfo("hgs--- in the special method to locate container")
            val idToHost = errorContainerIdToHost.iterator.next()
            val hosts:Array[String]  = idToHost._2 match {
                case Some(host)=> Array(host)
                case None => null
            }

            val rack = containerIdToRack.get(idToHost._1) match {
              case Some(list) => if(list==null) null else list.toArray
              case None => null
            }
            logInfo("hgs--the lost host:"+hosts)
            errorContainerIdToHost.remove(idToHost._1)
            containerIdToRack.remove(idToHost._1)
            logInfo("hgs--hosts:"+hosts.toList+"--rack:"+rack.toList)
            if(hosts==null || rack==null){
              if(hosts==null){
                logInfo("hgs--xxx:1--"+"nodes:"+nodes.toSeq+"--rrack:"+racks.toSeq)
                newLocalityRequests += createContainerRequest(resource, nodes, racks,true)
              }else{
                logInfo("hgs--xxx:2--"+hosts.toSeq+"rack:"+racks.toSeq)
                newLocalityRequests += newContainerRequestWithPriority(resource, hosts, racks,true)
              }
            }
            else {
              logInfo("hgs--xxx:3--"+hosts.toSeq+"--rack:"+rack.toSeq)
              newLocalityRequests += newContainerRequestWithPriority(resource, hosts, rack,false)
            }
          }else{
            logInfo("hgs-- in the normal method to locate containers.")
            newLocalityRequests += createContainerRequest(resource, nodes, racks,true)
          }
        case _ =>
      }
      ...
      if (availableContainers >= newLocalityRequests.size) {
        ...
        for (i <- 0 until (availableContainers - newLocalityRequests.size)) {
          if(hasFinishOnce&&errorContainerIdToHost.iterator.hasNext){
            logInfo("availableContainers--hgs--- in the special method to locate container")
             val idToHost: (ContainerId, Option[String]) = errorContainerIdToHost.iterator.next()
            val hosts:Array[String]  = idToHost._2 match{
              case Some(host) => Array(host)
              case None => null
            }

            val rack:Array[String] = containerIdToRack.get(idToHost._1) match {
                       case Some(list) => if(list==null) null else list.toArray
                       case None => null
            }
            logInfo("hgs--the lost host:"+hosts)
            errorContainerIdToHost.remove(idToHost._1)
            containerIdToRack.remove(idToHost._1)
            logInfo("hgs--hosts:"+hosts.toList+"--rack:"+rack)
            if(hosts==null || rack==null){
              if(hosts==null){
                logInfo("hgs--yyy:1--")
                newLocalityRequests += createContainerRequest(resource, null, null,true)
              }else{
                logInfo("hgs--yyy:2--"+hosts.toSeq)
                newLocalityRequests += newContainerRequestWithPriority(resource, hosts, null,true)

              }
            }else{
              logInfo("hgs--yyy:3"+"--hosts:"+hosts.toSeq+"--rack:"+rack.toSeq)
              newLocalityRequests += newContainerRequestWithPriority(resource, hosts, rack,false)
            }
          }else{
            logInfo("availableContainers--hgs-- in the normal method to locate .")
            newLocalityRequests += createContainerRequest(resource, null, null,true)
          }
  ...
  private def matchContainerToRequest(
    ...
    if (!matchingRequests.isEmpty) {
      //hgs begin
      var cnt = matchingRequests.size()
      for(i <- 0 until cnt-1){
        var it = matchingRequests.get(i).iterator();

        while(it.hasNext){
          val cn = it.next();
          logInfo("hgs--container msg:"+cn.getNodes+"--"+cn.getRacks)
      ...
      logInfo("hgs--container msg:"+containerRequest.getNodes+"--"+containerRequest.getRacks)
  ...
  private def runAllocatedContainers(containersToUse: ArrayBuffer[Container]): Unit = {
    for (container <- containersToUse) {
      ...
      logInfo(s"Launching container $containerId on host $executorHostname " +
        s"for execontainerIdToHostcutor with ID $executorId")
      containerIdToHost.put(containerId,executorHostname)
      var rack : String = null
      var isBreak:Boolean = false
      try{
        val iter = yarnClient.getNodeReports().iterator()
        while(iter.hasNext && !isBreak){
          val report = iter.next()
          logInfo("hgs-rack:"+report.getRackName+"--host:"+report.getNodeId.getHost+"--thishosts:"+executorHostname)
          if(executorHostname.equals(report.getNodeId.getHost)){
            rack = report.getRackName
            logInfo("hgs--the host:"+executorHostname+"  to rack-name is :"+rack)
            containerIdToRack.put(containerId,((rack.toString)::Nil))
            isBreak = true
          }
        }

      }catch{
        case ex => logInfo("hgs-- error when get the rack name to hosts:"+executorHostname+"--"+ex)
      }
      ...
      val exitReason = if (!alreadyReleased) {
        ...
        containerIdToExecutorId.get(containerId) match {
          case Some(executorId) => runningExecutors.remove(executorId)
                                  errorContainerIdToHost.put(completedContainer.getContainerId,hostOpt)
                                  logInfo("hgs--missing one container ,now add to error errorContainerIdToHost ." +
                                    " the map content" +
                                    errorContainerIdToHost)
